    ...
    if (!name.empty()) {
        ...
        if (-1 != m_fd) {
            ...
            if (0 < m_size) {
                ...
                if (!retVal) {
                    std::cerr << "[cluon::SharedMemory] Failed to truncate '" << m_name <<"': " << ::strerror(errno) << " (" << errno << ")" << std::endl;
                }
            }
            ...
            if (retVal) {
                ...
                if ( MAP_FAILED != m_sharedMemory) {
                    ...
                    std::cerr << "[cluon::SharedMemory] Failed to map '" << m_name <<"': " << ::strerror(errno) << " (" << errno << ")" << std::endl;
                }
